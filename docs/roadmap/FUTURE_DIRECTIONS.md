...
> **进度说明（截至 v1.3.0）**：第 1 节里的对齐线、快捷键、多端预览和嵌套容器，第 2 节的 `{{ }}` 表达式，第 3 节的 Vue SFC / Web Component / 独立 HTML 出码，以及第 4 节的版本备份与恢复，已经在当前代码里落地。下文仍有效的是还没做的部分：标尺、右键菜单、可视化联动规则、流程图、微前端发布、协同锁、远程物料插件。
